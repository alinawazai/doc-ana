...
# Function to handle saving the vector database (on button click)
def save_vector_db(documents):
    try:
...
def run_streamlit():
    ...
    if uploaded_pdf and not st.session_state.processed:
        if st.sidebar.button("Run Processing Pipeline"):
            ...
            gemini_documents = loop.run_until_complete(run_processing_pipeline(pdf_path))
            if gemini_documents:
    ...
    if st.session_state.processed and st.sidebar.button("Save Vector Database"):
        save_vector_db(st.session_state.gemini_documents)
    ...
    if query:
        ...
        if st.session_state.vector_db_saved:
            # Search and retrieve results from the vector database
            st.sidebar.write("Search results will be displayed here.")
        else:
            st.error("No vector database available for searching. Please process and save the vector database first.")
